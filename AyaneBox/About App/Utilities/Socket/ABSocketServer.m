...
@interface ABSocketServer () <GCDAsyncUdpSocketDelegate,GCDAsyncSocketDelegate>
...
- (void)startUDP {
    ...
    if (nil == self.udpServer) {
        ...
        if (![self.udpServer bindToPort:6002 error:&error]) {
...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didReceiveData:(NSData *)data fromAddress:(NSData *)address withFilterContext:(id)filterContext {
    ...
    if ([_delegate respondsToSelector:@selector(udpSocket:host:didReceiveData:fromAddress:port:)]) {
        [_delegate udpSocket:sock host:host didReceiveData:data fromAddress:address port:port];
    }
}
