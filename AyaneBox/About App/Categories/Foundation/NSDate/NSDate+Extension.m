...
- (NSString*)ageStr
{
    ...
    NSDateComponents *date = [calendar components:unitFlags fromDate:self toDate:currentDate options:0];
    ...
    {
        return [NSString stringWithFormat:(@"%ld月龄"),(long)[date month]];
    ...
    // 计算月龄
    NSInteger iAgeMonth = currentDateMonth - brithDateMonth - 1;
    if (currentDateDay >= brithDateDay) {
        iAgeMonth++;
    }
    
    // 计算天龄
    NSInteger iAgeDay = currentDateDay - brithDateDay;
    
    if (iAge != 0) {
        return [NSString stringWithFormat:@"%zd岁",iAge];
    }else if(iAgeMonth != 0){
        return [NSString stringWithFormat:@"%zd月龄",iAgeMonth];
    }else {
        return [NSString stringWithFormat:@"%zd天龄",iAgeDay];
    }
     */
}
...
+ (NSDateComponents*)componentsFromDate:(NSDate*)fromDate toDate:(NSDate*)toDate
{
    ...
    NSUInteger unitFlags = NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond ;
    //按照组成部分格式计算出生日期与现在时间的时间间隔
    NSDateComponents *date = [calendar components:unitFlags fromDate:fromDate toDate:toDate options:0];
    return date;
}
